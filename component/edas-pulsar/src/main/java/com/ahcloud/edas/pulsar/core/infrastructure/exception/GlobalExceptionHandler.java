package com.ahcloud.edas.pulsar.core.infrastructure.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseResult<Void> handleMaxUploadSizeExceededException(MaxUploadSizeExceededException e) {
        return ResponseResult.ofFailed(PulsarRetCodeEnum.PARAM_ILLEGAL);
    }
     ...
     * @return
     */
    @ExceptionHandler(PulsarAdminException.class)
    public ResponseResult<Void> handleCustomException(PulsarAdminException e) {
        return ResponseResult.ofFailed(e.getStatusCode(), e.getMessage());
    }
    ...
    public ResponseResult<Void> constraintViolationException(ConstraintViolationException e) {
        ...
        return ResponseResult.ofFailed(PulsarRetCodeEnum.PARAM_ILLEGAL.getCode(), errorMsg.toString());
    }
    ...
    public ResponseResult<Void> handleBindException(BindException ex) {
        ...
        String errorMsg = PulsarRetCodeEnum.PARAM_ILLEGAL.getMessage();
        if (Objects.nonNull(fieldError)) {
            errorMsg = fieldError.getDefaultMessage();
        }
        // 生成返回结果
        return ResponseResult.ofFailed(PulsarRetCodeEnum.PARAM_ILLEGAL.getCode(), errorMsg);
    }
}
